...
import {
  ...
  ActivityIndicator, FlatList, Keyboard, Modal,
} from 'react-native';
...
export default function LocationPickerModal({ visible, onConfirm, onClose }) {
      ...
      try {
        ...
        const enriched = await Promise.all(
          results.slice(0, 5).map(async (r) => {
            ...
            const a = rev[0];
            const label = [a?.name, a?.street, a?.city, a?.region, a?.country].filter(Boolean).join(', ');
    ...
    <Modal visible={visible} animationType="slide" presentationStyle="fullScreen" onRequestClose={handleClose}>
      <View style={[styles.container, { paddingTop: insets.top }]}>
        ...
        <View style={styles.header}>
          <TouchableOpacity onPress={handleClose} hitSlop={10} style={styles.backBtn}>
        ...
        <MapView
          ...
          showsCompass
        >
          {pin && <Marker coordinate={{ latitude: pin.latitude, longitude: pin.longitude }} pinColor={colors.accent} />}
        </MapView>
        ...
        {pin && (
          <View style={[styles.selectedBar, { paddingBottom: insets.bottom + 14 }]}>
...
function makeStyles(colors) {
  return StyleSheet.create({
    ...
    searchWrap: {
      ...
      backgroundColor: colors.bgHover, borderRadius: radius.lg, paddingLeft: 10,
    },
    ...
    suggestions: {
      ...
      borderRadius: radius.md, maxHeight: 220,
      borderWidth: 1, borderColor: colors.borderSubtle, zIndex: 10,
    },
    suggestionRow: {
